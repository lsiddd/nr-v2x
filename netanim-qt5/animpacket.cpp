...
static uint32_t gAnimId = INITIAL_ANIM_PACKET_ID;
QFont AnimPacket::gFont;
AnimPktMgr * AnimPktMgr::pAnimPktMgr = 0;
...

{
    ...
    m_graphicsItem = new AnimPacketLine;

    m_graphicsTextItem = new AnimPacketText;
    m_graphicsTextItem->setFlag(QGraphicsItem::ItemIgnoresTransformations);
    m_graphicsItem->pText = m_graphicsTextItem;
...
AnimPacket::parseArp(std::string metaInfo)
{
    ...
    arpInfo->type = rx.cap(1).toLatin1().data();
    arpInfo->sourceMac = rx.cap(2).toLatin1().data();
    arpInfo->sourceIpv4 = rx.cap(3).toLatin1().data();
    if( QString(rx.cap(4).toLatin1().data()) != "")
    arpInfo->destMac = rx.cap(4).toLatin1().data();
    arpInfo->destIpv4  = rx.cap(5).toLatin1().data();
    m_arpInfo = arpInfo;
...
AnimPacket::parseEthernet(std::string metaInfo)
{
    ...
    ethernetInfo->sourceMac = rx.cap(1).toLatin1().data();
    ethernetInfo->destMac = rx.cap(2).toLatin1().data();
...
AnimPacket::parseIcmp(std::string metaInfo)
{
    ...
    m_icmpInfo->type = rx.cap(1).toLatin1().data();
    m_icmpInfo->code = rx.cap(2).toLatin1().data();
...
AnimPacket::parseUdp(std::string metaInfo)
{
    ...
    m_udpInfo->length = rx.cap(1).toLatin1().data();
    m_udpInfo->SPort = rx.cap(2).toLatin1().data();
    m_udpInfo->DPort = rx.cap(3).toLatin1().data();
}
...
AnimPacket::parseIpv4(std::string metaInfo)
{
    ...
    m_ipv4Info->tos = rx.cap(1).toLatin1().data();
    m_ipv4Info->Dscp = rx.cap(2).toLatin1().data();
    m_ipv4Info->Ecn = rx.cap(3).toLatin1().data();
    m_ipv4Info->Ttl = rx.cap(4).toLatin1().data();
    m_ipv4Info->Id = rx.cap(5).toLatin1().data();
    m_ipv4Info->protocol = rx.cap(6).toLatin1().data();
    m_ipv4Info->length = rx.cap(7).toLatin1().data();
    m_ipv4Info->SrcIp = rx.cap(8).toLatin1().data();
    m_ipv4Info->DstIp = rx.cap(9).toLatin1().data();
}
...
AnimPacket::parseTcp(std::string metaInfo)
{
    ...
    m_tcpInfo->SPort = rx.cap(1).toLatin1().data();
    m_tcpInfo->DPort = rx.cap(2).toLatin1().data();
    m_tcpInfo->flags = rx.cap(3).toLatin1().data();
    m_tcpInfo->seq = rx.cap(4).toLatin1().data();
    m_tcpInfo->ack = rx.cap(5).toLatin1().data();
    m_tcpInfo->window = rx.cap(6).toLatin1().data();
}
...
AnimPacket::parseWifi(std::string metaInfo)
{
    ...
    if ((pos = rxCTL_ACK.indexIn(metaInfo.c_str())) != -1)
    {
        ...
        m_wifiMacInfo->Ra = rxCTL_ACK.cap(1).toLatin1().data();
       return;
    ...
    if ((pos = rxCTL_RTS.indexIn(metaInfo.c_str())) != -1)
    {
        ...
        m_wifiMacInfo->Ra = rxCTL_RTS.cap(1).toLatin1().data();
        m_wifiMacInfo->Sa = rxCTL_RTS.cap(2).toLatin1().data();
    ...
    if ((pos = rxCTL_CTS.indexIn(metaInfo.c_str())) != -1)
    {
        ...
        m_wifiMacInfo->Ra = rxCTL_CTS.cap(1).toLatin1().data();
        return;
    ...
    if ((pos = rx.indexIn(metaInfo.c_str())) == -1)
        return;
    m_wifiMacInfo = new WifiMacInfo;
    m_wifiMacInfo->type = rx.cap(1).toLatin1().data();
    m_wifiMacInfo->toDs = rx.cap(2).toLatin1().data();
    m_wifiMacInfo->fromDs = rx.cap(3).toLatin1().data();
    m_wifiMacInfo->Da = rx.cap(4).toLatin1().data();
    m_wifiMacInfo->Sa = rx.cap(5).toLatin1().data();
    m_wifiMacInfo->Bssid = rx.cap(6).toLatin1().data();
    ...
    if(m_wifiMacInfo->type == "MGT_ASSOCIATION_REQUEST")
    {
        ...
        if ((pos = rx.indexIn(metaInfo.c_str())) == -1)
            return;
        m_wifiMacInfo->SSid = rx.cap(1).toLatin1().data();
    }
    if(m_wifiMacInfo->type == "MGT_ASSOCIATION_RESPONSE")
    {
        ...
        if ((pos = rx.indexIn(metaInfo.c_str())) == -1)
            return;
        m_wifiMacInfo->assocResponseStatus = rx.cap(1).toLatin1().data();
    }
...
AnimPacket::parseAodv(std::string metaInfo)
{
    ...
    m_aodvInfo->type = rx.cap(1).toLatin1().data();
    if(m_aodvInfo->type == "RREQ")
    {
        ...
        if ((pos = rx.indexIn(metaInfo.c_str())) == -1)
            return;
        m_aodvInfo->destination = rx.cap(1).toLatin1().data();
        m_aodvInfo->seq = rx.cap(2).toLatin1().data();
        m_aodvInfo->source = rx.cap(1).toLatin1().data();
    ...
    if(m_aodvInfo->type == "RERR")
    {
        ...
        if ((pos = rx.indexIn(metaInfo.c_str())) == -1)
            return;
        m_aodvInfo->rerrInfo = rx.cap(1).toLatin1().data();
        m_aodvInfo->destination = rx.cap(2).toLatin1().data();
    }
...
AnimPacket::setTextTransform(std::string text, qreal angle)
{
    ...
    m_graphicsTextItem->setText(text.c_str());
...
AnimPktMgr::AnimPktMgr()
{

}

AnimPktMgr *
AnimPktMgr::getInstance()
{
    if(!pAnimPktMgr)
    {
        pAnimPktMgr = new AnimPktMgr;
    }
    return pAnimPktMgr;
}

AnimPacket * AnimPktMgr::addPacket(uint32_t fromId, uint32_t toId,
                         double fbTx, double lbTx, double fbRx,
                         double lbRx,
                         bool isWPacket)
{
...
AnimPktMgr::AnimPacketMap_t *
AnimPktMgr::getPackets()
{
    return &m_animPackets;
}


uint64_t
AnimPktMgr::getPacketCount()
{
    return m_animPackets.size();
}

void
AnimPktMgr::systemReset()
{
    for (AnimPktMgr::AnimPacketMap_t::const_iterator i = m_animPackets.begin();
         i != m_animPackets.end();
         ++i)
    {
        delete (i->second);
    ...
    m_animPackets.clear();
    gAnimId = INITIAL_ANIM_PACKET_ID;
    m_metaInfoSeen = false;
}

uint64_t
AnimPktMgr::getNextAnimId()
{
    return gAnimId++;
}
...
void
AnimPktMgr::setMetaInfoSeen()
{
    m_metaInfoSeen = true;
}

bool
AnimPktMgr::getMetaInfoSeen()
{
    return m_metaInfoSeen;
}

void
AnimPacketLine::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
...
void
AnimPacketText::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    painter->save();
    if(!m_isWest)
    {
        painter->translate(0, -15 * AnimatorView::getInstance()->getCurrentZoomFactor());// * AnimatorView::getInstance()->transform().m11());
    }
    else
    {
        painter->translate(0, 5 * AnimatorView::getInstance()->getCurrentZoomFactor());
    }

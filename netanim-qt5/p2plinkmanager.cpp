...
P2PLinkManager::applyLinkItem(DesignerP2PLinkLine *linkItem)
{
    ...
    p2pHelper.SetDeviceAttribute("DataRate", ns3::StringValue (linkItem->getDataRate().toLatin1().data()));
    p2pHelper.SetChannelAttribute("Delay", ns3::StringValue (linkItem->getDelay().toLatin1().data()));

    ns3::NetDeviceContainer devices;
    devices = p2pHelper.Install (nodeContainer);

    ns3::Ipv4AddressHelper address;
    address.SetBase (linkItem->getIpv4Base().toLatin1().data(),
                     linkItem->getIpv4Mask().toLatin1().data());

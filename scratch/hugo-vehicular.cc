...
static void
Rx (Ptr<OutputStreamWrapper> stream, Ptr<const Packet> p)
{
  SeqTsHeader header;
  ...
  *stream->GetStream () << Simulator::Now ().GetSeconds () << "\t" << p->GetSize () << "\t"
                        << header.GetSeq () << "\t" << header.GetTs ().GetSeconds () << std::endl;
}

void
print_position (NodeContainer ueNodes)
{
  std::cout << '\n';
  std::cout << "--------------------------------" << std::endl;
  ...
  for (uint32_t j = 0; j < ueNodes.GetN (); ++j)
    {
      ...
      std::cout << " Node " << j << " | POS: (x=" << pos.x << ", y=" << pos.y << ") | Speed("
                << speed.x << ", " << speed.y << ")" << std::endl;
    }
  Simulator::Schedule (Seconds (1), &print_position, ueNodes);
}

void
send_data (Ptr<Node> sender, Ptr<Node> receiver, double availableRate)
{
  static uint16_t port_1 = 4000; // well-known echo port number
  static uint16_t port_2 = 4001;

  UdpEchoServerHelper server (port_1);

  ApplicationContainer apps = server.Install (sender);
  ...
  ApplicationContainer apps2 = server2.Install (sender);
  ...
  UdpClientHelper client (sender->GetObject<Ipv4> ()->GetAddress (1, 0).GetLocal (), port_1);
  client.SetAttribute ("MaxPackets", UintegerValue (maxPacketCount));
  client.SetAttribute ("Interval", TimeValue (interPacketInterval));
  client.SetAttribute ("PacketSize", UintegerValue (packetSize));
  ApplicationContainer clientApps = client.Install (receiver);
...
main (int argc, char *argv[])
{
  ...
  Config::SetDefault ("ns3::MmWaveVehicularPropagationLossModel::ChannelCondition",
                      StringValue ("l"));
  Config::SetDefault ("ns3::MmWaveVehicularSpectrumPropagationLossModel::Frequency",
                      DoubleValue (28.0e9));
  ...
  NS_LOG_DEBUG (
      "IPv4 Address node 0: " << group.Get (0)->GetObject<Ipv4> ()->GetAddress (1, 0).GetLocal ());
  NS_LOG_DEBUG (
      "IPv4 Address node 1: " << group.Get (1)->GetObject<Ipv4> ()->GetAddress (1, 0).GetLocal ());
  NS_LOG_DEBUG (
      "IPv4 Address node 2: " << group.Get (2)->GetObject<Ipv4> ()->GetAddress (1, 0).GetLocal ());

  Ptr<mmwave::MmWaveAmc> m_amc =
      CreateObject<mmwave::MmWaveAmc> (helper->GetConfigurationParameters ());
  double availableRate = m_amc->GetTbSizeFromMcsSymbols (28, 14) / 0.001; // bps

  Ipv4GlobalRoutingHelper::PopulateRoutingTables ();

  Simulator::Schedule (Seconds (1), &send_data, group.Get (0), group.Get (1), availableRate);
  Simulator::Schedule (Seconds (2), &send_data, group.Get (1), group.Get (2), availableRate);
  Simulator::Schedule (Seconds (3), &send_data, group.Get (2), group.Get (0), availableRate);

  Simulator::Schedule (Seconds (0.1), &print_position, group);
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin ();
       i != stats.end (); ++i)
    {
      ...
      if (i->second.rxBytes)
        std::cout << "  DelaySum: "
                  << i->second.jitterSum / (i->second.rxPackets + i->second.txPackets) << "\n";
      std::cout << "......................................\n";
    }
